...
if ( is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
	...
	if ( ! function_exists( 'print_attribute_radio' ) ) {
		function print_attribute_radio( $checked_value, $value, $label, $name ) {
			...
			printf( '<div><input type="radio" name="%1$s" value="%2$s" id="%3$s" %4$s><label for="%3$s" data-cursor-hover="true" 
			data-cursor-hover-background="rgba(105,102,255,0,678)">%5$s</label></div>', $input_name, $esc_value, $id, $checked, $filtered_label );
		}
	}
}
