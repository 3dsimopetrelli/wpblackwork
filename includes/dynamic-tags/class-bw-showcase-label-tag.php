<?php
/**
 * Dynamic Tag per la Showcase Label dei prodotti WooCommerce.
 */
...
if ( class_exists( '\Elementor\Core\DynamicTags\Tag' ) && class_exists( '\Elementor\Modules\DynamicTags\Module' ) ) {
	...
	/**
	 * Dynamic Tag Elementor: Showcase Label.
	 */
	class BW_Showcase_Label_Tag extends \Elementor\Core\DynamicTags\Tag {
		...
		/**
		 * Recupera la Showcase Label dal metadato del prodotto corrente.
		 *
		...
		public function get_value( array $options = [] ) {
			...
			$showcase_label = get_post_meta( $post_id, '_bw_showcase_label', true );

			return is_string( $showcase_label ) ? $showcase_label : '';
		}
	...
	/**
	 * Registra la dynamic tag personalizzata in Elementor.
	 *
	 * @param \Elementor\Core\DynamicTags\Manager $dynamic_tags Gestore delle dynamic tags.
	 */
	function bw_register_showcase_label_dynamic_tag( $dynamic_tags ) {
		$dynamic_tags->register( new BW_Showcase_Label_Tag() );
	}
	add_action( 'elementor/dynamic_tags/register', 'bw_register_showcase_label_dynamic_tag' );
}
