...
class BW_Klarna_Gateway extends BW_Abstract_Stripe_Gateway {
	...
	public function process_payment( $order_id ) {
		...
		switch ( $status ) {
			case 'succeeded':
				if ( ! $order->is_paid() ) {
					$order->update_status( 'on-hold', sprintf( __( 'Klarna return received. Awaiting Stripe webhook confirmation. PaymentIntent: %s', 'bw' ), $pi_id ) );
				}
	...
	public function get_icon() {
		return '<span class="bw-payment-method__klarna-logo" role="img" aria-label="' . esc_attr__( 'Klarna', 'bw' ) . '"></span>';
	}
