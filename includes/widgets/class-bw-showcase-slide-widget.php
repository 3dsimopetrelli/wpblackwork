...
class BW_Showcase_Slide_Widget extends Widget_Base {
	...
	public function get_script_depends() {
		return array( 'embla-js', 'embla-autoplay-js', 'bw-embla-core-js', 'bw-showcase-slide-script' );
	}

	public function get_style_depends() {
		return array( 'bw-embla-core-css', 'bw-showcase-slide-style' );
	}
	...
	private function register_style_controls() {
		...
		BW_Widget_Helper::add_typography_group( $this, 'title_typography', '{{WRAPPER}} .bw-showcase-slide-title' );
		...
		BW_Widget_Helper::add_typography_group( $this, 'subtitle_typography', '{{WRAPPER}} .bw-showcase-slide-description' );
		...
		BW_Widget_Helper::add_typography_group( $this, 'labels_typography', '{{WRAPPER}} .bw-showcase-slide-badge' );
		...
		BW_Widget_Helper::add_typography_group( $this, 'physical_info_typography', '{{WRAPPER}} .bw-showcase-slide-physical-line' );
		...
		BW_Widget_Helper::add_typography_group( $this, 'link_button_typography', '{{WRAPPER}} .bw-showcase-slide-button' );
		...
		BW_Widget_Helper::add_dimensions_control(
			...
			'{{WRAPPER}} .bw-showcase-slide-media, {{WRAPPER}} .bw-showcase-slide-image img',
			'border-radius',
			array( 'px', '%' )
		);
						...
						<?php elseif ( ! empty( $slide['labels'] ) ) : ?>
							<div class="bw-showcase-slide-labels">
								<?php foreach ( $slide['labels'] as $index => $label ) : ?>
									<?php
									$badge_classes = array( 'bw-showcase-slide-badge' );
									if ( $index < 2 ) {
	...
	private function build_physical_info_list( $all_meta ) {
		...
		foreach ( array( '_bw_info_1', '_bw_info_2' ) as $info_key ) {
	...
	protected function render_breakpoint_css( $settings ) {
		...
		foreach ( $breakpoints as $bp ) {
			...
			if ( ! $frame_ratio && in_array( $height_mode, array( 'fixed', 'contain', 'cover' ), true ) ) {
				$css .= $sel_horizontal . '{';
				...
				if ( in_array( $height_mode, array( 'contain', 'cover' ), true ) && ! empty( $image_width['size'] ) ) {
			...
			if ( ! $frame_ratio && in_array( $height_mode, array( 'contain', 'cover' ), true ) && ! empty( $image_width['size'] ) ) {
