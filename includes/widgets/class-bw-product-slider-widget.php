...
class BW_Product_Slider_Widget extends Widget_Base {
	...
	public function get_script_depends() {
		return array( 'embla-js', 'embla-autoplay-js', 'bw-embla-core-js', 'bw-product-slider-script' );
	}

	public function get_style_depends() {
		return array( 'bw-product-card-style', 'bw-embla-core-css', 'bw-product-slider-style' );
	}
	...
	protected function register_controls() {
		...
		BW_Widget_Helper::add_dimensions_control(
			...
			'{{WRAPPER}} .bw-slider-image-container',
			'border-radius',
			array( 'px', '%' )
		);
		...
		BW_Widget_Helper::add_dimensions_control(
			...
			'{{WRAPPER}} .bw-product-card .bw-wallpost-title, {{WRAPPER}} .bw-product-card .bw-slider-title',
			'padding',
			array( 'px', 'em', '%' )
		);
		...
		BW_Widget_Helper::add_dimensions_control(
			...
			'price_padding',
			__( 'Padding', 'bw-elementor-widgets' ),
			'{{WRAPPER}} .bw-product-card .bw-wallpost-price, {{WRAPPER}} .bw-product-card .bw-slider-price, {{WRAPPER}} .bw-product-card .price',
			'padding',
			array( 'px', 'em', '%' )
		);
		...
		BW_Widget_Helper::add_color_var_control(
			$this,
			'card_buttons_bg',
			__( 'Background', 'bw-elementor-widgets' ),
			'{{WRAPPER}}',
			'--bw-card-overlay-buttons-background',
			'#FFFFFF'
		);
		...
		BW_Widget_Helper::add_color_var_control(
			$this,
			'card_buttons_color',
			__( 'Text Color', 'bw-elementor-widgets' ),
			'{{WRAPPER}}',
			'--bw-card-overlay-buttons-color',
			'#000000'
		);
		...
		BW_Widget_Helper::add_color_var_control(
			...
			__( 'Background (Hover)', 'bw-elementor-widgets' ),
			'{{WRAPPER}}',
			'--bw-card-overlay-buttons-background-hover',
			'#80FD03'
		);
		...
		BW_Widget_Helper::add_color_var_control(
			...
			__( 'Text Color (Hover)', 'bw-elementor-widgets' ),
			'{{WRAPPER}}',
			'--bw-card-overlay-buttons-color-hover',
			'#000000'
		);
	...
	protected function render() {
		...
		if ( ! empty( $settings['specific_ids'] ) ) {
			...
			if ( ! empty( $settings['subcategory'] ) ) {
				$subcats     = is_array( $settings['subcategory'] ) ? $settings['subcategory'] : array( $settings['subcategory'] );
				$tax_query[] = array(
				...
				$parents     = is_array( $settings['parent_category'] ) ? $settings['parent_category'] : array( $settings['parent_category'] );
				$tax_query[] = array(
					'taxonomy'         => 'product_cat',
					'field'            => 'term_id',
