...
class Widget_Bw_Product_Breadcrumbs extends Widget_Base {
	...
	public function get_style_depends() {
		if ( function_exists( 'bw_register_widget_assets' ) && ! wp_style_is( 'bw-product-breadcrumbs-style', 'registered' ) ) {
			bw_register_widget_assets( 'product-breadcrumbs', array(), false );
		}

		return array( 'bw-product-breadcrumbs-style' );
	}
	...
	protected function register_controls() {
		...
		BW_Widget_Helper::add_dimensions_control(
			...
			'padding',
			__( 'Padding', 'bw' ),
			'{{WRAPPER}} .bw-product-breadcrumbs',
			'padding',
			array( 'px' ),
			array(
				'top'      => 0,
		...
		BW_Widget_Helper::add_typography_group(
			$this,
			'typography',
			'{{WRAPPER}} .bw-product-breadcrumbs__item, {{WRAPPER}} .bw-product-breadcrumbs__link, {{WRAPPER}} .bw-product-breadcrumbs__current',
			array(
	...
	protected function render() {
		...
		$breadcrumbs = $product_id > 0 ? $this->build_breadcrumbs( $product_id ) : array();
	...
	private function resolve_product_id( array $settings ): int {
		...
		if ( ! $product_id && function_exists( 'bw_tbl_resolve_product_context_id' ) ) {
			$resolution = bw_tbl_resolve_product_context_id( array_merge( $settings, array( '__widget_class' => __CLASS__ ) ) );
