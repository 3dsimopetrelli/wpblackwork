...
if ( ! function_exists( 'bw_mf_filter_product_admin_thumbnail_size' ) ) {
	function bw_mf_filter_product_admin_thumbnail_size( $size ) {
		...
		return array( $thumb_size, $thumb_size );
	}
}
...
if ( ! function_exists( 'bw_mf_admin_is_media_library_screen' ) ) {
	function bw_mf_admin_is_media_library_screen() {
		if ( ! is_admin() ) {
			return false;
		}
...
if ( ! function_exists( 'bw_mf_get_active_filter_payload' ) ) {
	function bw_mf_get_active_filter_payload( $post_type ) {
		...
		if ( $post_type !== 'attachment' || ! in_array( $mode, array( 'list', 'grid' ), true ) ) {
			$mode = 'list';
		}
...
if ( ! function_exists( 'bw_mf_admin_enqueue_assets' ) ) {
	function bw_mf_admin_enqueue_assets( $hook_suffix = '' ) {
		...
		$is_media_library_screen = bw_mf_admin_is_media_library_screen();
		$post_type               = $is_list_screen ? bw_mf_get_current_screen_post_type() : ( $is_media_library_screen ? 'attachment' : '' );
		if ( $post_type === '' ) {
			$post_type = $is_media_library_screen ? 'attachment' : '';
		}
		$screen_context = $is_list_screen
			? bw_mf_get_context_for_post_type( $post_type )
			: ( $is_media_library_screen ? 'upload' : '' );
...
if ( ! function_exists( 'bw_mf_add_drag_handle_column' ) ) {
	function bw_mf_add_drag_handle_column( $columns ) {
		...
		$drag_priority_keys = $post_type === 'product'
			? array( 'name', 'title', 'cb' )
			: array( 'title', 'cb' );
		$copy_priority_keys = array( 'author', 'comments', 'date' );
...
if ( ! function_exists( 'bw_mf_render_drag_handle_column' ) ) {
	function bw_mf_render_drag_handle_column( $column_name, $post_id ) {
		if ( ! in_array( $column_name, array( 'bw_mf_drag_handle', 'bw_mf_copy_link' ), true ) ) {
			return;
		}
