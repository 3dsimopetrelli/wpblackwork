...
class SAS_Blog extends Widget_Base {
	...
	public function render_loop_item() {
		...
		$tmp_settings = array(
			'enable_navigation' => $settings['enable_grid_navigation'], 
			...
			'blog_posts_offset' => $settings['blog_posts_offset'],
			'preview_words' => $settings['preview_words']
		);

		$wp_query = sas_get_posts($tmp_settings,$paged);
	...
	public function render_loop_item_load_more() {
		...
		$settings = $this->get_settings();
		$id       = 'sas-grid_id_' . $this->get_id();
		$category = $wp_query->get("category_name") ? $wp_query->get("category_name") : false;
		...
		if($category) {
			$tmp_settings['category_name'] = $category;
		}
