...
Arrow and dots visibility per breakpoint is managed **entirely by CSS `@media` rules** generated by PHP (`render_breakpoint_css()`) and emitted inline inside the widget markup, not by JavaScript.
