...
### 6.6 ✅ RISOLTO — Stripe internal "Card + icon" subview

**Soluzione adottata (finale):**
- usare `layout.type = tabs` nel controllo supportato `wc_stripe_upe_params`.
- evitare hack selector-based su nodi interni Stripe come soluzione primaria.

**Razionale:**
- i subview interni del Payment Element sono UI Stripe-owned;
- i tentativi di hide via selector interni sono risultati fragili/non deterministici;
- `tabs` ha risolto il duplicato in modo UX-safe sul surface supportato.

**Maintenance note:**
- se il problema riappare dopo aggiornamenti Stripe/Woo, verificare prima la configurazione layout del Payment Element;
- la configurazione preferita resta `tabs`;
- non reintrodurre hack su selector interni Stripe come fix principale.
