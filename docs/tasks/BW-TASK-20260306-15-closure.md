...
- Hardened `bw_register_product_grid_widget_assets()` to prevent duplicate `wp_localize_script()` execution when the function is triggered by multiple hooks.
...
## Verification Steps
1. Confirm the static guard exists in `bw_register_product_grid_widget_assets()`.
2. Confirm `wp_localize_script('bw-product-grid-js', 'bwProductGridAjax', ...)` is wrapped by the one-time guard.
3. Run:
