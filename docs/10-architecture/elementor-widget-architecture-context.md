...
- `bw-trust-box` is now the standalone trust/support authority for the lower column stack:
  - global review slider
  - global fixed review box
  - widget-local digital product info cards
  - widget-local FAQ CTA
- shared slider runtime reuse applies there too: the curated trust slider is implemented through the existing Embla core rather than a widget-specific carousel stack.
