...
$current_view = isset( $_GET['bw-view'] ) ? sanitize_key( wp_unslash( $_GET['bw-view'] ) ) : '';

$endpoint_urls = [
    'orders'   => $orders_url,
    'invoices' => add_query_arg( 'bw-view', 'invoices', $orders_url ),
];
...
$icons = [
    ...
    'invoices'         => '<svg aria-hidden="true" width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6 2h9l3 3v17l-2-1-2 1-2-1-2 1-2-1-2 1V2z" stroke="currentColor" stroke-width="2" fill="none"/><path d="M8 8h8" stroke="currentColor" stroke-width="2"/><path d="M8 12h8" stroke="currentColor" stroke-width="2"/><path d="M8 16h5" stroke="currentColor" stroke-width="2"/></svg>',
...
<nav class="woocommerce-MyAccount-navigation">
    <ul>
        <?php foreach ( $menu_items as $endpoint => $label ) :
            $is_orders_view = ( 'orders' === $current );
            $is_invoices    = ( 'invoices' === $endpoint ) && $is_orders_view && ( 'invoices' === $current_view );
            $is_purchases   = ( 'orders' === $endpoint ) && $is_orders_view && '' === $current_view;
            $is_active      = $is_invoices || $is_purchases || ( $endpoint === $current ) || ( '' === $endpoint && ! $current );
            $endpoint_url   = $endpoint_urls[ $endpoint ] ?? wc_get_account_endpoint_url( $endpoint );
            ?>
